...
Milter-Recipe is yamilter, a milter built from pluggable recipes
(`Milter::Recipe::*`) on top of Sendmail::PMilter, plus the tools for testing
recipes: a milter client, a harness that runs yamilter, and `yamilter-corpus`,
which replays real mail through recipes and stores the verdicts in SQLite.
Read `lib/Milter/Recipe.pm` first, then `lib/Milter/Client.pm`.
...
| `prereqs.yml` | the prerequisites `[AutoPrereqs]` cannot see, and the minimum perl |
| GitHub issues | known bugs, including in recipes that are not fixed yet |

Tests which run yamilter need Sendmail::PMilter and Lingua::Identify.  The
corpus tests use the fixture mail in `t/corpus/`, which uses `test.test`
addresses only; never add real mail there.
